...
def check_dataset(adata):
    ...
    assert "mode2_noisy" in adata.obsm
    assert "mode2_obs" in adata.uns
    assert np.all(adata.obs.index == adata.uns["mode2_obs"])

    assert adata.obsm["mode2"].shape == adata.obsm["mode2_noisy"].shape
...
def sample_dataset():
    ...
    adata.obsm["mode2_noisy"] = adata.obsm["mode2"]
...
def sample_method(adata):
    ...
    adata.obsm["mode2_denoised"] = adata.obsm["mode2"]
    return adata
