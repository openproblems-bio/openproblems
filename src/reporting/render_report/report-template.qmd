...
source_columns <- c("commit", "source_urls", "common_dataset_names", "date_created")
...
scores <- purrr::map_dfr(task_results$results, function(.result) {
  ...
  tibble::tibble(
    ...
    raw_value = unlist(.result$metric_values)
  )
}) |>
  dplyr::mutate(
    value = dplyr::if_else(metric %in% metric_reverse, -raw_value, raw_value)
  )
...
#| results: hide
#| eval: !expr has_controls
fig_height <- max(c(length(metric_names), 10))
